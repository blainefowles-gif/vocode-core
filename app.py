...
#   REALTIME_MODEL   (optional, defaults to gpt-4o-realtime-preview)
#
...
async def voice(_: Request):
    """Tell Twilio to open a bidirectional audio stream to /media."""
...
# -------------------------------------------------------------------
# Helper: μ-law 8 kHz → PCM16 24 kHz converter
# -------------------------------------------------------------------
...
async def media(ws: WebSocket):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.ws_connect(
            ...
            # Configure realtime session (Server VAD enabled)
            await oai_ws.send_json({
                ...
                "session": {
                    ...
                    "voice": "alloy",
                    # 👇 Server-side VAD auto-commits + auto-responses
                    "turn_detection": {
                        "type": "server_vad",
                        "threshold": 0.5,
                ...
                async for msg in oai_ws:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        if typ == "response.audio.delta":
                            # g711_ulaw@8 kHz chunk for Twilio
